...
export function Explorer({
  ...
  // FR-61: the manual execution trace (an ordered route the user clicks together),
  // its status narration, and the trail the surfaces paint — all owned by the hook so
  // the shell stays lean (mirroring useGraphDiff / useIngest). View-only (FR-9);
  // playback reuses the FR-40/FR-48 replay through the surface controller.
  const { trace, traceStatus, traceSteps, onTraceClick, undoStep, clearTrace, playTrace } =
    useManualTrace({ nodes, edges, labelFor, traceArmed, controllerRef });
